...
#
# A slightly more interactive stock trading simulation. 
# (Refactoring of Question 1)
#
...
def Main():
    d = getStockData()
    m = eval(input("Please enter the Downward Trend (Sell) threshold (m): "))
    n = eval(input("Please enter the Upward Trend (Buy) threshold (n): "))
    print("\t{}\t{}\t{}\t{}".format("m", "n", "Trans", "Balance"))
    transactions, balance = invest(d, m, n)
    print("{0}\t{1}\t{2}\t{3}\t${4:.2f}".format(0,m,n,transactions,balance))
